...
public class TagGroup<T> extends ViewGroup {
    ...
    public TagGroup(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TagCropGroup);
        mVerticalSpacing = a.getDimension(R.styleable.TagCropGroup_tgp_vertical_spacing, 10);
        mHorizontalSpacing = a.getDimension(R.styleable.TagCropGroup_tgp_horizontal_spacing, 10);
        mMaxRow = a.getInt(R.styleable.TagCropGroup_tgp_max_row, 10);
        a.recycle();
    ...
    /*public void setList(List<DemKind> tags, int layout) {
        removeAllViews();

        for (int i = 0; i < tags.size(); i++) {
            View view = null;
            if (kindTagViewFactory != null) {
                view = kindTagViewFactory.create(this, i, tags.get(i));
            } else {
                view = LayoutInflater.from(getContext()).inflate(layout, null, false);
                ((TextView)view).setWidth(Device.dp2px(75));
                ((TextView) view).setText(tags.get(i).name.toString());
            }
            int j = i;
    ...
    /*public void setKindTagViewFactory(DemKindTagViewFactory view){
        this.mKindTagViewFactory=view;
    }*/

    private TagViewFactory<T> mTagViewFactory;
    /*private DemKindTagViewFactory mKindTagViewFactory;*/
    ...
    public interface DemKindTagViewFactory {
        View create(ViewGroup group, int position, DemKind tag);
    }
