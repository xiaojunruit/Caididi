 ...
 */
public class AddCategoryAnimator {
    ...
    public AddCategoryAnimator() {
    }

    public AddCategoryAnimator with(View view) {
        return with((Activity) view.getRootView().getContext(), view);
    }

    public AddCategoryAnimator with(Activity activity, View view) {
    ...
    private AddCategoryAnimator setOriginRect(float x, float y) {
        originX = x;
        originY = y;
        return this;
    }

    private AddCategoryAnimator setDestRect(float x, float y) {
        destX = x;
        destY = y;

        LogUtil.e("desx" + x + "y:" + y);
        return this;
    }

    public AddCategoryAnimator to(View view) {
    ...
    public AddCategoryAnimator to(View view, int position) {
        int mar = Device.dp2px(15);
        int x = 0;
        if (position > 0) {
            x = (int) (view.getX() + view.getWidth() + mar);
        }
        view.getLocationOnScreen(dest);
        dest[0] = x;
        setDestRect(view.getWidth(), view.getHeight());
        return this;
    }
    ...
    private boolean prepare() {
        if (mRefContext.get() != null) {
            ViewGroup decoreView = (ViewGroup) mRefContext.get().getWindow().getDecorView();

            mBitmap = drawViewToBitmap(mTarget, mTarget.getWidth(), mTarget.getHeight());
            if (mImageView == null) {
                mImageView = new ImageView(mRefContext.get());
            }
    ...
    private AnimatorSet getAvatarRevealAnimator() {
        ...
        animatorCircleSet.setDuration(mScaleDuration);
...
//                mDest.getLocationOnScreen(dest);
                ...
                Animator translatorX = ObjectAnimator.ofFloat(mImageView, View.X, x, x + dest[0] - (src[0] + (originX * scaleFactor - 2 * endRadius
                        * scaleFactor) / 2) + (0.5f * destX - scaleFactor * endRadius));
                ...
                Animator translatorY = ObjectAnimator.ofFloat(mImageView, View.Y, y, y + dest[1] - (src[1] + (originY * scaleFactor - 2 * endRadius
                        * scaleFactor) / 2) + (0.5f * destY - scaleFactor * endRadius));
    ...
    public AddCategoryAnimator setOnAnimatorEndListener(AnimatorEndListener listener) {
